...
def select_features(splits, seed: int) -> list[str]:
    ...
    model.fit(X, y)
    return [cols[i] for i in np.argsort(-model.feature_importances_)[:N_FEATURES]]
...
def main() -> None:
    ...
    for cap in [int(c) for c in args.bond_caps.split(",")]:
        ...
        ctrl, ctrl_step, _ = train(mps_random(N_FEATURES, info["max_bins"], cap, args.seed, E_va[:1]),
                                   E_tr, y_tr_t, E_va, y_va_t, args.steps, args.lr)
        ctrl_auprc = test_eval(ctrl)
        rnd = random_cores(N_FEATURES, info["max_bins"], cap, args.seed)
