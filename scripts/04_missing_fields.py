...
def main() -> None:
    ...
    features = [all_cols[i] for i in np.argsort(-importance_model.feature_importances_)[:N_FEATURES]]
    ...
    for rate in MISSING_RATES:
        ...
        for k in range(N_MASK_SEEDS):
            val_mask = mask_for_rate(len(X_val), N_FEATURES, rate, seed=1000 * (k + 1) + args.seed)
            test_mask = mask_for_rate(len(X_test), N_FEATURES, rate, seed=2000 * (k + 1) + args.seed)
