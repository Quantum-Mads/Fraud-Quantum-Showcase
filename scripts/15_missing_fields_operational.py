...
def main() -> None:
    ...
    features = [all_cols[i] for i in np.argsort(-imp.feature_importances_)[:s04.N_FEATURES]]
    ...
    no_mask_val = np.zeros((len(X_val), s04.N_FEATURES), dtype=bool)
    ...
    no_mask_test = np.zeros((len(X_test), s04.N_FEATURES), dtype=bool)
    for rate in [0.0] + s04.MISSING_RATES:
        ...
        for k in range(n_masks):
            if rate == 0.0:
                ...
                vmask = s04.mask_for_rate(len(X_val), s04.N_FEATURES, rate, seed=1000 * (k + 1) + SEED)
                tmask = s04.mask_for_rate(len(X_test), s04.N_FEATURES, rate, seed=2000 * (k + 1) + SEED)
